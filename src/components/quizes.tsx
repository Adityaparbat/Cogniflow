...
import { QuizQuestion, getRandomQuizQuestions, quizData } from '@/lib/quizData';
...
export default function Quiz({ onClose, onComplete, subject, questionCount = 10, chapter, usePDFContent = false }: QuizProps) {
  ...
  useEffect(() => {
      ...
      if (usePDFContent && subject && chapter) {
        try {
          ...
          if (module.generatedQuizzes && module.generatedQuizzes.length > 0) {
            quizQuestions = module.generatedQuizzes;
            setContentSource('pdf');
          }
        } catch (error) {
          console.log('No PDF content found, using regular quiz data');
        }
      }
      
      if (quizQuestions.length === 0) {
        // Use regular quiz data
        quizQuestions = quizData;
        if (subject) {
          quizQuestions = quizData.filter(q => q.subject === subject);
        }
    ...
    loadQuestions();
  }, [questionCount, subject, chapter, usePDFContent]);
        ...
        <div className="bg-white rounded-2xl p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">
            {usePDFContent && subject && chapter 
              ? `No quiz questions found for ${subject} - ${chapter}. Try uploading a PDF first.`
              : 'No quiz questions available for the selected subject.'
            }
          </p>
          <button
            onClick={onClose}
